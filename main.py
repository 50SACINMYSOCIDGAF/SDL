...
class ManifoldAwareAutoencoder(nn.Module):
    ...
    def compute_nonlinearity_score(self, x: torch.Tensor) -> torch.Tensor:
        ...
        skewness = torch.mean(torch.pow(centered, 3), dim=1) / (torch.var(sim_matrix, dim=1) + self.eps) ** 1.5
        manifold_score = self.manifold_pattern_detector(x).squeeze(-1)
        signal_weights_norm = F.softmax(self.signal_weights, dim=0)
        combined_signal = signal_weights_norm[0] * row_variance + signal_weights_norm[1] * entropy + signal_weights_norm[2] * torch.abs(skewness)
        nonlinearity_score = torch.sigmoid((combined_signal - 0.5 * combined_signal.mean()) / 0.1)
        temp = 2.0
        boosted_score = torch.log(torch.exp(nonlinearity_score * temp) + torch.exp(manifold_score * temp)) / temp
    ...
    def compute_feature_statistics(self, activations: torch.Tensor):
        ...
        with autocast(device_type, enabled=False):
            ...
            coherence_mean = torch.abs(gram[mask]).mean().item() if mask.any() else 0.0
            act_centered = act - act.mean(dim=0, keepdim=True)
            cov_matrix = (act_centered.T @ act_centered) / N
            reg_strength = max(1e-6, torch.max(torch.abs(cov_matrix)).item() * 1e-6)
            eye = torch.eye(cov_matrix.size(0), device=cov_matrix.device)
            cov_matrix = cov_matrix + reg_strength * eye
            try:
                with torch.no_grad():
                    eigenvals = torch.linalg.eigvalsh(cov_matrix)
                    eigenvals = torch.clamp(eigenvals, min=self.eps)
                    eigenvals_sum = torch.sum(eigenvals)
                    normalized_eigenvals = eigenvals / (eigenvals_sum + self.eps)
                    diversity = 1.0 - torch.max(normalized_eigenvals) / (torch.sum(normalized_eigenvals) + self.eps)
                    diversity = diversity.item()
            except Exception:
                diversity = 0.5
            active_mask = (act > 0).float()
            stability_metric = torch.zeros(1, device=act.device)
            if act.size(0) > 1:
                feature_overlap = torch.mm(active_mask, active_mask.t())
                diag_mask = ~torch.eye(act.size(0), dtype=torch.bool, device=act.device)
                overlap_ratio = feature_overlap[diag_mask].view(act.size(0), act.size(0) - 1) / (active_mask.sum(dim=1, keepdim=True) + self.eps)
                stability_metric = overlap_ratio.mean()
            stability = max(0.05, stability_metric.item())
            nonlinearity = self.last_nonlinearity_score.mean().item() if self.last_nonlinearity_score is not None else 0.0
            gate_utilization = (1.0 - self.last_gates).mean().item() if hasattr(self, 'last_gates') else 0.0
...
class SparseDictionaryTrainer:
    def __init__(self, model_path: str, layer_dims, device: str = "cuda", beta: float = 0.98):
        ...
        self.saes = nn.ModuleList([ManifoldAwareAutoencoder(self.hidden_size, hidden_dim).to(self.device) for _, hidden_dim in layer_dims])
        self.beta = beta
        self.difficulty_history = []
        self.best_metrics = {}
        self.checkpoint_dir = "checkpoints"
        os.makedirs(self.checkpoint_dir, exist_ok=True)
        wandb.init(project="sparse_dictionary_learning", config={"layers": layer_dims, "model": model_path, "beta": beta, "model_type": "manifold_aware"})
    ...
    def prepare_dataset(self, dataset_name="wikitext", subset="wikitext-103-v1", max_samples=50000):
        dataset = load_dataset(dataset_name, subset)
        if max_samples and "train" in dataset and len(dataset["train"]) > max_samples:
            dataset["train"] = dataset["train"].select(range(max_samples))
        print(f"Dataset loaded: {len(dataset['train'])} training samples")
        return dataset
    def create_dataloaders(self, dataset, batch_size=32, val_split=0.1, max_length=512, num_workers=4, max_train_batches=100, max_val_batches=20):
        def tokenize_function(examples):
            return self.tokenizer(examples["text"], padding="max_length", truncation=True, max_length=max_length, return_tensors="pt")
        tokenized_dataset = dataset.map(tokenize_function, batched=True, batch_size=1000, remove_columns=["text"] if "text" in dataset["train"].column_names else None)
        tokenized_dataset.set_format("torch")
        if "validation" not in tokenized_dataset:
            train_size = int((1 - val_split) * len(tokenized_dataset["train"]))
        ...
        if max_train_batches > 0:
            required_samples = max_train_batches * batch_size
            if len(train_dataset) > required_samples:
                import random
                indices = random.sample(range(len(train_dataset)), required_samples)
                train_dataset = Subset(train_dataset, indices)
        if max_val_batches > 0:
            required_samples = max_val_batches * batch_size
            if len(val_dataset) > required_samples:
                import random
                indices = random.sample(range(len(val_dataset)), required_samples)
        ...
        print(f"Created dataloaders: {len(train_dataloader)} training batches, {len(val_dataloader)} validation batches")
    ...
    def train_layer(self, layer_idx: int, train_data, val_data, epochs: int = 50, batch_size: int = 64):
        ...
        scaler = GradScaler()
        loss_scaler = DynamicLossScaler({"reconstruction": 1.0, "sparsity": 0.1, "diversity": 0.05, "stability": 0.025, "orthogonality": 0.01, "nonlinearity": 0.1, "gate_diversity": 0.05})
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                ...
                with autocast(self.device, dtype=dtype):
                    ...
                    loss = (scales["reconstruction"] * recon_loss + scales["sparsity"] * sparsity_penalty * frame_potential + scales["diversity"] * (1 - diversity) + scales["stability"] * (1 - stability) + scales["orthogonality"] * ortho_loss + scales["gate_diversity"] * gate_diversity_loss + scales.get("nonlinearity", 0.1) * (1 - nonlinearity)) / grad_accum_steps
                    metrics = {"loss": loss.item() * grad_accum_steps, "sparsity": sparsity.item(), "diversity": diversity, "stability": stability, "recon": recon_loss.item(), "ortho": ortho_loss.item(), "gate_div": gate_diversity_loss.item(), "nonlinearity": nonlinearity}
    ...
    def validate(self, layer_idx: int, val_data) -> FeatureMetrics:
        ...
        with torch.no_grad():
            for batch in val_data:
                ...
                batch_metrics = FeatureMetrics(float(sparsity.item()), float(coherence), float(stability), float(diversity), float(semantic), float(recon_loss.item()), float(ortho_loss.item()), float(nonlinearity))
...
if __name__ == "__main__":
    ...
    if args.dummy_data:
        train_dataloader, val_dataloader = trainer.use_dummy_data(train_batches=args.train_batches, val_batches=args.val_batches, batch_size=args.batch_size, seq_length=args.max_length)
    else:
        dataset = trainer.prepare_dataset(dataset_name=args.dataset, subset=args.subset, max_samples=args.max_samples)
        train_dataloader, val_dataloader = trainer.create_dataloaders(dataset, batch_size=args.batch_size, max_length=args.max_length, max_train_batches=args.train_batches, max_val_batches=args.val_batches)
